...
import { useRiskTimeline } from "@/features/risk/hooks/use-risk-timeline";
...
export default function AnalyticsPage(){
  ...
  const riskTimeline=useRiskTimeline(location,hours,3);
  ...
  const riskData=useMemo(()=>riskTimeline.data?.points.map(point=>({name:forecastTime(point.assessment_time,"Asia/Kolkata"),risk:point.score,level:point.level,quality:point.data_quality}))??[],[riskTimeline.data]);
  return <div className="page">
    ...
    <div className="chart-grid"><ChartCard title={`Marine Operational Risk · ${hours}H`} status="CURRENT"><div className="chart-wrap">{riskTimeline.loading?<div className="risk-timeline-state">Calculating deterministic forecast risk…</div>:riskTimeline.error?<div className="risk-timeline-state error">{riskTimeline.error}</div>:riskData.length?<ResponsiveContainer width="100%" height="100%"><AreaChart data={riskData}><defs><linearGradient id="risk-score" x1="0" x2="0" y1="0" y2="1"><stop offset="0%" stopColor="#e36d31" stopOpacity={.3}/><stop offset="100%" stopColor="#e36d31" stopOpacity={.02}/></linearGradient></defs><XAxis dataKey="name" axisLine={false} tickLine={false} tick={{fontSize:10,fill:"#627d98"}}/><YAxis domain={[0,100]} axisLine={false} tickLine={false} tick={{fontSize:10,fill:"#627d98"}} width={34}/><Tooltip/><Area connectNulls={false} dataKey="risk" stroke="#e36d31" strokeWidth={2} fill="url(#risk-score)"/></AreaChart></ResponsiveContainer>:<div className="risk-timeline-state">Select a location to calculate the timeline.</div>}</div><p className="chart-footer">{riskTimeline.data?.risk_model_version??"ORCA deterministic risk"} · one forecast retrieval evaluated locally at 3-hour intervals. Missing scores remain gaps.</p></ChartCard>{charts.map(chart=><ChartCard title={`${chart.title} · ${hours}H`} status="CURRENT" key={chart.key}><div className="chart-wrap"><ResponsiveContainer width="100%" height="100%"><AreaChart data={chartData}><defs><linearGradient id={chart.key} x1="0" x2="0" y1="0" y2="1"><stop offset="0%" stopColor={chart.color} stopOpacity={.28}/><stop offset="100%" stopColor={chart.color} stopOpacity={.01}/></linearGradient></defs><XAxis dataKey="name" axisLine={false} tickLine={false} tick={{fontSize:10,fill:"#627d98"}}/><YAxis axisLine={false} tickLine={false} tick={{fontSize:10,fill:"#627d98"}} width={34}/><Tooltip/><Area connectNulls={false} dataKey={chart.key} stroke={chart.color} strokeWidth={2} fill={`url(#${chart.key})`}/></AreaChart></ResponsiveContainer></div><p className="chart-footer">Model forecast · units: {chart.unit}. Missing provider values are shown as gaps.</p></ChartCard>)}</div>
  </div>;
}
