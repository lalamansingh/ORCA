...
export const evaluateRisk=(payload:RiskEvaluationPayload)=>apiClient<MarineRiskAssessment>(`${API_V1_PREFIX}/risk/evaluate`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload),timeoutMs:45_000});

export const getQuickRisk=(payload:RiskEvaluationPayload)=>{
  const query=new URLSearchParams({latitude:String(payload.latitude),longitude:String(payload.longitude),...(payload.assessment_time?{assessment_time:payload.assessment_time}:{}),...(payload.refresh?{refresh:"true"}:{})});
