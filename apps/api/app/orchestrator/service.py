...
class ORCAOrchestrator:
    ...
    async def execute(self,plan:ExecutionPlan,raw_query:str="",selected_location:dict|None=None)->OrchestrationResult:
        ...
        return OrchestrationResult(query_id=plan.query_id,trace_id=trace_id,status=result_status,intent=plan.primary_intent,location=state.get("selected_location"),requested_time=plan.requested_time,data={key:value for key,value in state.items() if key in {"weather","marine","alerts","risk","pfz","ocean_products","geospatial","geofence"}},evidence=state.get("evidence",[]),step_results=results,warnings=state.get("warnings",[]),errors=errors,map_actions=[MapAction.model_validate(action) for action in state.get("map_actions",[])],started_at=started,completed_at=datetime.now(UTC),duration_ms=(time.perf_counter()-clock)*1000)
