...
class GeofenceService:
    ...
    async def check_point(self,latitude:float,longitude:float,activity:str="UNKNOWN",assessment_time:datetime|None=None,radius_km:float=10)->GeofenceAssessment:
        ...
        demo = any((zone.metadata_ or {}).get("demo") or "demo" in zone.source.lower() or "fixture" in zone.source.lower() for zone in inside + [zone for zone, _ in nearby])
        ...
        return GeofenceAssessment(location={"latitude":latitude,"longitude":longitude},overall_status=overall,inside_zones=contained,nearby_zones=near,nearest_boundary=near[0] if near else (contained[0] if contained else None),warnings=[f"Within {self.caution_km:g} km of a mapped zone boundary."] if overall==GeofenceStatus.CAUTION else [],restrictions=[rule for item in contained for rule in item.restrictions if rule.get("activity") in {activity,"ALL","UNKNOWN"}],evidence=evidence,assessed_at=now,data_status="DEMO" if demo else "OPERATIONAL",limitations=[LEGAL_NOTICE])
